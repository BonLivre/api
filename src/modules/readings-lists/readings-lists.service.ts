...
export class ReadingsListsService {
  ...
  async createReadingsList(
    user: User,
    createReadingsListDto: CreateReadingsListDto
  ...
  private async formatReadingsList(
    ...
    const books = await Promise.all(
      readingsList.books.map((book) => this.booksService.formatBook(book))
    );
    ...
    return {
      ...
      photoUrl: readingsList.user.photo,
    };
  }

  async findReadingsListsById(
    readingsListId: string,
    user?: User
  ): Promise<ReadingsListDto> {
    ...
    return this.formatReadingsList(
      readingsList,
      user?.id === readingsList.userId
    );
  }
  ...
  async findReadingsLists(currentUser?: User): Promise<FindReadingsListssDto> {
    ...
    const data: ReadingsListDto[] = await Promise.all(
      readingsLists.map(async (readingsList) =>
        this.formatReadingsList(
          readingsList,
          currentUser?.id === readingsList.userId
        )
      )
    );
  ...
  async findReadingsListsIOwn(
    currentUser: User
  ): Promise<FindReadingsListssDto> {
    const readingsLists = await this.prisma.readingsList.findMany({
  ...
  async toggleBookPresenceInReadingsList(
    currentUser: User,
    readingsListId: string,
    bookId: string
  ) {
